                ...
                <div class="col-md-4">
                    <div class="media">
                        <div class="media-body">
                            <div class="display-6">
                                TABEL<b>Pemesanan</b></div>
                            <p class="small text-muted">
                                tabel pemesanan ikan</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-8">
                    <div class="pull-xs-right" role="toolbar"><a class="btn btn-icon icon-only"><i class="fa fa-rss"></i></a><a class="btn btn-icon icon-only"><i class="fa fa-star text-warning"></i></a><a class="btn btn-success" href="https://github.com/wenzhixin/bootstrap-table" target="_blank">Official website</a></div>
                </div>
            </div>
        </div>
        <!-- END PAGE HEADER-->
                            ...
                            <div class="panel-body">
                                <table class="table" data-plugin="DataTable">
                                    ...
                                    <?php foreach ($data as $row){ ?>
                                        <form>
                                            <input type="hidden" id="kualitas" value="<?php echo $row->kualitas; ?>">
                                            <input type="hidden" id="hasil" value="<?php echo $row->hasil; ?>">
                                        </form>
                                    <tr>
                                        <td><?php echo $row->nama_ikan; ?></td>
                                        <td><?php echo $row->ket_usia; ?></td>
                                        <td><?php echo $row->ket_ukuran_ikan; ?></td>
                                        <td><?php echo $row->tanggal_pesan; ?></td>
                                        <td><?php echo $row->jumlah; ?></td>
                                        <td><?php echo $row->status_pemesanan; ?></td>
                                        <td>
                                            <?php
                                            if ($row->id_status_pemesanan == 3){
                                                echo '<button type="button" onclick="klik(\'Pemesanan anda mendekati Kualitas '.$row->kualitas.' dengan nilai '.$row->hasil.'. Klik Yes Untuk Memesan Ulang\', \' '.base_url('index.php/pemesanan/batalPesan/'.$row->id_pemesanan).'\', \' '.base_url('index.php/pemesanan/pesanUlang/'.$row->id_pemesanan).'\')" class="btn m-b-2 btn-youtube"><i class="fa icon fa-search"></i></button>';
                                            } else if ($row->id_status_pemesanan == 1){
                                                echo '<a href="'.base_url('index.php/pemesanan/getFormedit/'.$row->id_pemesanan).'" type="button" class="btn m-b-2 btn-whatsapp"><i class="fa icon fa-pencil-square-o"></i></a>';
                                                echo '<button onclick="hapus(\'Anda Yakin ingin menghapus pemesanan ini ?\', \' '.base_url('index.php/pemesanan/deletePesan/'.$row->id_pemesanan).'/pemesanan\')" class="btn m-b-2 btn-youtube" type="button"><i class="fa icon fa-times"></i></button>';
                                            } else {
                                                echo "";
                                            }
                                            ?>
        ...
        <script>
            function klik(pesan,url,tolak) {
                swal({
                        ...
                        confirmButtonText: "Ya, Pesan Kembali",
